...
from .models.groq.interface import init_groq
...
def ba_agent_groq(model, defination):
    requirements_finder = init_groq(model)
...
def po_agent_groq(model, defination, requirements):
    requirements_finder = init_groq(model)
    ...
    with open("./docs/po_result.txt", "w") as f:
        f.write(result)

    return result


def pm_agent_groq(model, goals):
    task_decomposer = init_groq(model)
...
def ssd_agent_groq(model, num_of_devs, documentation, ssd_request):
    task_decomposer = init_groq(model)
    ...
    template = f"""
                ...
                Output Format (JSON): 
                Return the structured response in a One JSON array with the following keys:
